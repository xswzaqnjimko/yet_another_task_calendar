...
fn delete_task(task_id: String, state: State<DbState>) -> Result<(), String> {
    ...
    // First, get all occurrence IDs for this task
    let mut stmt = conn
        .prepare("SELECT id FROM occurrences WHERE task_id = ?1")
        .map_err(|e| e.to_string())?;
    
    let occurrence_ids: Vec<String> = stmt
        .query_map([&task_id], |row| row.get(0))
        .map_err(|e| e.to_string())?
        .filter_map(|r| r.ok())
        .collect();
    
    // Delete time entries for all occurrences of this task
    for occ_id in &occurrence_ids {
        conn.execute("DELETE FROM time_entries WHERE occurrence_id = ?1", [occ_id])
            .map_err(|e| e.to_string())?;
    }
    
    // Delete all occurrences for this task
    conn.execute("DELETE FROM occurrences WHERE task_id = ?1", [&task_id])
        .map_err(|e| e.to_string())?;
...
fn main() {
    tauri::Builder::default()
        .setup(|_app| {
            // Use Application Support directly (not app-specific subfolder)
            // to match existing database location
            ...
            std::fs::create_dir_all(&app_support).expect("Failed to create app directory");
            
            let db_path = app_support.join("task_grid.db");
